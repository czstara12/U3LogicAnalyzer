...
void usage()
{
	fprintf(stdout,
		...
		"      --smoke-test                Verify Qt, libraries and protocol decoders, then exit\n"
		"\n", PV_BIN_NAME);
}
using namespace std;
...
#ifdef ENABLE_DECODE
/**
 * @brief 设置协议解码运行环境，使便携包启动不依赖当前工作目录。
 * @details 搜索 Windows 便携目录、Linux 安装前缀和 macOS 应用资源目录。
 *          保留用户显式设置的环境变量。
 */
static void prepare_decode_environment()
{
	...
	if (qEnvironmentVariableIsEmpty("PYTHONHOME")) {
		...
		candidates << app_dir.filePath("python") << app_dir.filePath("python-portable");
#ifdef Q_OS_MAC
		candidates << app_dir.filePath("../Resources");
#elif !defined(Q_OS_WIN)
		candidates << app_dir.filePath("..");
#endif
		for (const QString &candidate : candidates) {
			const QDir candidate_dir(candidate);
			const QStringList libraries = QDir(candidate_dir.filePath("lib")).entryList(
				QStringList("python3.*"), QDir::Dirs | QDir::NoDotAndDotDot);
			bool has_stdlib = QDir(candidate_dir.filePath("Lib/encodings")).exists();
			for (const QString &library : libraries)
				has_stdlib = has_stdlib || QDir(candidate_dir.filePath(
					"lib/" + library + "/encodings")).exists();
			if (has_stdlib) {
				python_home = candidate_dir.absolutePath();
				break;
			}
		}
		if (python_home.isEmpty()) {
			QFile home_file(app_dir.filePath("python-home.txt"));
			if (home_file.open(QIODevice::ReadOnly | QIODevice::Text))
				python_home = QString::fromUtf8(home_file.readAll()).trimmed();
		}
		if (!python_home.isEmpty() && QDir(python_home).exists())
			qputenv("PYTHONHOME", QDir::toNativeSeparators(python_home).toUtf8());
	}
	if (qEnvironmentVariableIsEmpty("SIGROKDECODE_DIR")) {
		const QStringList candidates = {
			"decoders", "share/libsigrokdecode/decoders",
			"../Resources/share/libsigrokdecode/decoders",
			"../share/libsigrokdecode/decoders",
			"../../install/share/libsigrokdecode/decoders"
		};
		for (const QString &candidate : candidates) {
			const QDir decoder_dir(app_dir.filePath(candidate));
			if (decoder_dir.exists()) {
				qputenv("SIGROKDECODE_DIR", QDir::toNativeSeparators(
					decoder_dir.absolutePath()).toUtf8());
				break;
			}
		}
	}
...
int main(int argc, char *argv[])
{
	...
	while (true) {
		static const struct option long_options[] = {
			...
			{"smoke-test", no_argument, nullptr, 'T'},
	...
	do {
		...
		if (smoke_test) {
#ifdef ENABLE_DECODE
			const unsigned int decoder_count = g_slist_length(
				const_cast<GSList *>(srd_decoder_list()));
			if (decoder_count == 0) {
				qCritical() << "未加载任何协议解码器。";
				ret = 1;
			} else {
				qInfo() << "启动自检通过，已加载协议解码器：" << decoder_count;
			}
			srd_exit();
#else
			qInfo() << "启动自检通过，当前构建未启用协议解码。";
#endif
			break;
		}
