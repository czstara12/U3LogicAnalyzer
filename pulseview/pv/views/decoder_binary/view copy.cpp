	...
	signal_(nullptr)
{
	...
	// 允许工具栏缩小 - 设置水平策略为 Ignored 表示忽略 sizeHint
	...
	// 设置 parent QMainWindow 允许缩小
	parent->setMinimumWidth(10);
	parent->setSizePolicy(QSizePolicy(QSizePolicy::Ignored, QSizePolicy::Expanding));
...
void View::clear_decode_signals()
{
	...
	for (const shared_ptr<DecodeSignal>& signal : decode_signals_) {
		...
		// 如果当前选中的信号是这个，断开 new_binary_data 信号
	...
	// 先断开 decoder_selector_ 的信号，防止 clear() 触发回调时数据状态不一致
	disconnect(decoder_selector_, SIGNAL(currentIndexChanged(int)),
		this, SLOT(on_selected_decoder_changed(int)));
	disconnect(class_selector_, SIGNAL(currentIndexChanged(int)),
		this, SLOT(on_selected_class_changed(int)));
...
void View::remove_decode_signal(shared_ptr<data::DecodeSignal> signal)
{
	...
	// 先断开 decoder_selector_ 和 class_selector_ 的信号，防止修改时触发回调
	disconnect(decoder_selector_, SIGNAL(currentIndexChanged(int)),
		this, SLOT(on_selected_decoder_changed(int)));
	disconnect(class_selector_, SIGNAL(currentIndexChanged(int)),
		this, SLOT(on_selected_class_changed(int)));
...
{
	// 当开始新采集时，只重置数据状态，保留解码器选择
	if (state == Session::Running) {
		// 停止定时器，清除数据显示，但保留 signal_ 和 decoder_ 指针
		delayed_view_updater_.stop();
		binary_data_exists_ = false;
		hex_view_->clear();
	}
