 ...
 * all drivers.
 */
#ifdef SR_CMAKE_DRIVER_LIST
SR_PRIV extern const struct sr_dev_driver *demo_driver_info_list[];
#ifdef HAVE_HW_WCH_CH32H417
SR_PRIV extern const struct sr_dev_driver *ch32h417_driver_info_list[];
#endif
#else
...
#ifdef HAVE_DRIVERS
#ifdef SR_CMAKE_DRIVER_LIST
	const struct sr_dev_driver **lists[] = {
#ifdef HAVE_HW_WCH_CH32H417
		ch32h417_driver_info_list,
#endif
		demo_driver_info_list,
	};

	for (size_t i = 0; i < ARRAY_SIZE(lists); i++) {
		for (const struct sr_dev_driver **driver = lists[i]; *driver; driver++)
			g_array_append_val(array, *driver);
	}
#else
